(ns cips_china.main
  "Kotodama WASM entrypoint for the Cips_china clean-room actor (L4) — Clojure port.
  ...
  facts (`cips_china.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cips_china")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"cips_china.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cips_china-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cips_china-compat/ready)
